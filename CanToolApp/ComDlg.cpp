...
BOOL CComDlg::OnInitDialog()
{
	...
	((CComboBox*)GetDlgItem(IDC_COM_RATE))->SetCurSel(6);
	((CComboBox*)GetDlgItem(IDC_COM_DATABIT))->SetCurSel(3);
	((CComboBox*)GetDlgItem(IDC_COM_STOPBIT))->SetCurSel(0);
	((CComboBox*)GetDlgItem(IDC_CAN_RATE))->SetCurSel(0);
	if (((CMainDlg *)GetParent())->m_ctrlComm.get_PortOpen())//如过已经打开串口，
		GetDlgItem(IDOK)->SetWindowTextW(L"取消连接");
	else
		GetDlgItem(IDOK)->SetWindowTextW(L"连接");
...
void CComDlg::OnBnClickedOk()
{
	...
	if (((CMainDlg *)GetParent())->m_ctrlComm.get_PortOpen())//如过已经打开串口，
	{
		((CMainDlg *)GetParent())->m_ctrlComm.put_PortOpen(false);//打开串口
		GetDlgItem(IDOK)->SetWindowTextW(L"连接");
		return;
	}
	...
	************/
	...
	Sleep(5000);
	((CMainDlg *)GetParent())->sendComData(L"C\r");
	Sleep(1000);
	m_Index = ((CComboBox*)GetDlgItem(IDC_CAN_RATE))->GetCurSel();
	str.Format( L"S%d\r", m_Index);
	((CMainDlg *)GetParent())->sendComData(str);
	Sleep(1000);
	((CMainDlg *)GetParent())->sendComData(L"O1\r");
	Sleep(1000);
	((CMainDlg *)GetParent())->sendComData(L"Ol\r");
